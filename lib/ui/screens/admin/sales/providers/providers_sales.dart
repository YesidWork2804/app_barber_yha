...
import '../../../../../infrasctructure/products/product_repositorie.dart';
...
//LISTA DE PRODUCTOS DISPONIBLES
  ...
  List<Product> _productosDisponibles = []; // Lista de productos disponibles
  // List<Product> _productosDisponiblesBackup = [];
  ...
  Future<List<Product>> initProducts() async {
    List<Product> productos = await repository.getproducts();
    // _productosDisponiblesBackup = productos;
  ...
  List<Product> getProductosDisponibles() {
    return List.from(_productosDisponibles); // Retorna una copia de la lista
  }
...
//LISTA DE PRODUCTOS
  ...
  List<Product> get productosDisponiblesFiltrados {
    return _productosDisponibles
        .where((product) => !_productosSeleccionados.contains(product))
        .toList();
  }
  ...
  Future<void> removerDisponibles(Product producto) async {
    // Simulación de una operación asíncrona (puede ser una llamada a una API)
    await Future.delayed(const Duration(seconds: 1));
    _productosDisponibles.remove(producto);
    notifyListeners();
